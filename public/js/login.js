...
document.getElementById("form-recuperar").onsubmit = async (e) => {
    ...
    try {
        ...
        if (res.ok) {
            // Sucesso
            Swal.fire("Sucesso!", "Link de recuperação enviado! Verifique seu e-mail.", "success");
            ...
            Swal.fire("Aviso!", dados.erro || "Erro ao tentar enviar o link.", "warning");
        }
    } catch (error) {
        console.error("Erro:", error);
        Swal.fire("Erro!", "Ocorreu um erro de comunicação com o servidor. Tente novamente.", "error");
    } finally {
        btn.disabled = false;
